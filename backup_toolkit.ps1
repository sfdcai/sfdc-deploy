# toolkit.ps1 - Definitive Final Interactive Version
...
function Show-CreditHeader {
    ...
    try {
        ...
        if ($width) {
            ...
            $paddingString = " " * [math]::Max(0, $padding)
...
function Select-Project {
    ...
    $configs = Get-ChildItem -Path $Global:ConfigDir -Filter "*.json"
    ...
    if ($choice -match '^\d+$') {
        ...
        if ($idx -ge 0 -and $idx -lt $configs.Count) {
            ...
            $Global:ProjectConfigFile = Join-Path -Path $Global:ConfigDir -ChildPath "$($newProjectName).json"
        } else {
...
$Global:LogFilePath = Join-Path -Path $Global:ConfigDir -ChildPath "$($Global:ProjectName).log"
...
function Show-SystemCheck {
    ...
    $settings = Get-ToolkitSettings
    $cacheValid = $false
    if ($settings.SystemInfo -and $settings.SystemInfo.LastCheck -and $settings.SystemInfo.LastCheck -as [datetime]) {
        if ((New-TimeSpan -Start ([datetime]$settings.SystemInfo.LastCheck) -End (Get-Date)).TotalHours -lt 24) {
            $cacheValid = $true
        }
    }
    ...
    if ($cacheValid -and -not $ForceRefresh) {
        ...
        $settings.SystemInfo = @{ LastCheck = (Get-Date -Format "o"); Software = $liveSoftwareStatus }
        Set-ToolkitSettings $settings
